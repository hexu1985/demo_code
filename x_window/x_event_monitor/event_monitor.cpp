...
void EventMonitor::handleRecordEvent(XRecordInterceptData* data)
{
    if (data->category == XRecordFromServer) {
        ...
        case ButtonRelease:
            ...
            break;
        case KeyPress:
            keyPress(((unsigned char*) data->data)[1]);
            
            break;
        case KeyRelease:
            keyRelease(((unsigned char*) data->data)[1]);
...
void EventMonitor::buttonPress(int x, int y)
{
    std::cout << "Button press (" << x << ", " << y << ")" << std::endl;
}

void EventMonitor::buttonDrag(int x, int y)
{
    std::cout << "Button drag (" << x << ", " << y << ")" << std::endl;
}

void EventMonitor::buttonRelease(int x, int y)
{
    std::cout << "Button release (" << x << ", " << y << ")" << std::endl;
}

void EventMonitor::keyPress(int code)
{
    std::cout << "Key press " << code << std::endl;
}

void EventMonitor::keyRelease(int code)
{
