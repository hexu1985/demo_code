...
if ! iconv -f GB18030 -t UTF-8 ${OUT_DIR}/index.html > ${OUT_DIR}/index_utf8.html
then
  echo "iconv error"
  exit 5
fi
...
MAX_PAGE_LINE=$(grep '<span class="all">.*</span>' ${OUT_DIR}/index_utf8.html)
echo ${MAX_PAGE_LINE}
...
# get max page number
MAX_PAGE=$(echo ${MAX_PAGE_LINE} | sed 's/.*共\(.*\)页.*/\1/g')
echo ${MAX_PAGE}
