...
int main(int argc, char *argv[])
{
	...
	while (1){
		...
		if (tcpptr->syn && !tcpptr->ack) {
			if (!send_tcp_syn_ack(tcpptr, ipptr, pppoe, vlan, vlan_num, eptr, send_descr)) {
				cerr << "send_tcp_syn_ack fault!\n";
				exit(1);
			}
			continue;
		}
		...
		if (tcpptr->fin) {
			if (!send_tcp_fin_ack(tcpptr, ipptr, pppoe, vlan, vlan_num, eptr, send_descr)) {
				cerr << "send_tcp_fin_ack fault!\n";
				exit(1);
			}
		...
		if (data_off > http_resp.pbuf->size()) {
			...
			cout << "out of range\n" << endl;
			if (!send_tcp_rst(tcpptr, ipptr, pppoe, vlan, vlan_num, eptr, send_descr)) {
				cerr << "send_tcp_fin fault!\n";
				exit(1);
			}
			http_resp_map.erase(iter);
			continue;
		}
		if (!send_tcp_data(http_resp.pbuf->data() + data_off, 
