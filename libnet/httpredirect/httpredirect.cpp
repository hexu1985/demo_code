...
int main(int argc, char *argv[])
{
	...
	const char *redirect_url = "http://www.qq.com/";

	if (argc < 2){
		printf("Usage: ipsniffer <interface> [filter host] [redirect url]\n");
		exit(1);
	}
	...
	cout << "redirect_url: " << redirect_url << endl;
	...
	// build http 302 resp
	string http_302_data = build_http_302(redirect_url);
...
#else
			if (!send_tcp_resp(http_302_data.c_str(), http_302_data.length(),
