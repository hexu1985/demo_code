		...
		struct ether_header *eptr, libnet_t *l)
{
	...
	t = libnet_build_tcp(
			...
			ntohl(0),			                   /* sequence number */
			ntohl(tcpptr->seq)+1,                  /* acknowledgement num */
			0x12,                                  /* control flags */
			17520,                                 /* window size */
			0,                                     /* checksum */
			0,                                     /* urgent pointer */
			LIBNET_TCP_H + 4,                      /* TCP packet size */
			NULL,                                  /* payload */
			0,                                     /* payload size */
			l,                                     /* libnet handle */
			0);                                    /* libnet id */
	...
	// 构造IP datagram
	t = libnet_build_ipv4(
			LIBNET_IPV4_H + LIBNET_TCP_H + 4,        /* length */
			0,                                       /* TOS */
			1691,                                    /* IP ID */
			0,                                       /* IP Frag */
			64,                                      /* TTL */
	...
	if (pppoe != NULL) {
		...
		ppphdr = NULL;
		pppoe_payload_len = 0;
		pppoe_payload_len = LIBNET_TCP_H + LIBNET_IPV4_H + 4 + 2;
