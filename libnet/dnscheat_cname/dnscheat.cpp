...
vector<uint8_t> build_dns4_answer(const char *org_name,
	uint8_t *cname, int len, int ttl, uint32_t addr);
...
int main(int argc, char *argv[])
{
	...
	vector<uint8_t> dns_answer = build_dns4_answer("www.baidu1.com", baidu_cname, sizeof (baidu_cname), 3600, dns_ans_ip);
 ...
 */
void build_dns4_resp(uint8_t *dns_query, int query_len, 
		uint8_t *dns_answer, int answer_len, 
		uint8_t *buffer, int *buffer_len)
{
	...
	dnsptr->num_answ_rr = htons((uint16_t) 2);
 ...
 */
vector<uint8_t> build_dns4_answer(const char *org_name,
		uint8_t *cname, int len,
		int ttl, uint32_t addr)
{
	vector<uint8_t> buffer;
	buffer.resize(28+len);	// answer RR size

	// fake dns answer
	// fake cname
	uint8_t *ans_rr1 = &buffer[0];	// answer RR begin
	*(uint16_t *)(ans_rr1) = htons((uint16_t) 0xc00c); // name
	*(uint16_t *)(ans_rr1+2) = htons((uint16_t) 0x0005); // type
	*(uint16_t *)(ans_rr1+4) = htons((uint16_t) 0x0001); // class
    *(uint32_t *)(ans_rr1+6) = htonl((uint32_t) ttl);    // ttl
	*(uint16_t *)(ans_rr1+10) = htons((uint16_t) len); // rdlength
	memcpy(ans_rr1+12, cname, len);

	// fake dns answer
	uint8_t *ans_rr2 = ans_rr1+12+len;	// answer RR begin
//	*(uint16_t *)(ans_rr1) = htons((uint16_t) 0xc018); // name
	// name
	*(uint8_t *)(ans_rr2) = (uint8_t) 0xc0;
	*(uint8_t *)(ans_rr2+1) = (uint8_t)(12+12+4+strlen(org_name)+2);
	*(uint16_t *)(ans_rr2+2) = htons((uint16_t) 0x0001); // type
	*(uint16_t *)(ans_rr2+4) = htons((uint16_t) 0x0001); // class
    *(uint32_t *)(ans_rr2+6) = htonl((uint32_t) ttl);    // ttl
	*(uint16_t *)(ans_rr2+10) = htons((uint16_t) 0x0004); // rdlength
	*(uint32_t *)(ans_rr2+12) = addr; // addr
	return buffer;
}
